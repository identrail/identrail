...
- For Kubernetes provider runs, keep `IDENTRAIL_K8S_SOURCE=kubectl`, apply `rbac-scanner-readonly.example.yaml`, patch API/worker to use `identrail-scanner`, and use an image that includes `kubectl`.
