...
func TestPostgresStoreFindingQueryHelpers(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("sqlmock: %v", err)
	}
	...
	if clause := findingsOrderClause("severity", true); !strings.Contains(clause, "WHEN 'critical'") || !strings.Contains(clause, "DESC") {
		t.Fatalf("unexpected severity order clause: %q", clause)
	}
	if clause := findingsOrderClause("title", false); !strings.Contains(clause, "f.title ASC") {
		t.Fatalf("unexpected title order clause: %q", clause)
	}
	if !errorsIsNoRows(sql.ErrNoRows) || errorsIsNoRows(fmt.Errorf("wrapped: %w", sql.ErrNoRows)) || errorsIsNoRows(errors.New("other")) {
		t.Fatal("expected errorsIsNoRows to match only the direct sql.ErrNoRows sentinel")
	}

	pageRows := sqlmock.NewRows([]string{"scan_id", "finding_id", "type", "severity", "title", "human_summary", "path", "evidence", "remediation", "created_at"}).
		AddRow("scan-1", "finding-1", "escalation_path", "critical", "Alpha", "summary", []byte("[\"x\"]"), []byte("{\"a\":1}"), "fix", now)
	mock.ExpectQuery(`SELECT f\.scan_id, f\.finding_id, f\.type, f\.severity, f\.title, f\.human_summary, f\.path, f\.evidence, COALESCE\(f\.remediation, ''\), f\.created_at`).
		WithArgs("", "critical", "escalation_path", "ack", "sec-oncall", "default", "default", 10, 2).
		WillReturnRows(pageRows)

	page, err := store.ListFindingsPage(defaultScopeContext(), FindingsPageQuery{
		Severity:        "Critical",
	...
	if err != nil {
		t.Fatalf("list findings page: %v", err)
	}
	if len(page) != 1 || page[0].ID != "finding-1" {
	...
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT EXISTS (
		...
		WithArgs("scan-1", "default", "default").
		WillReturnRows(sqlmock.NewRows([]string{"exists"}).AddRow(true))
	findingByScanRows := sqlmock.NewRows([]string{"scan_id", "finding_id", "type", "severity", "title", "human_summary", "path", "evidence", "remediation", "created_at"}).
		AddRow("scan-1", "finding-1", "ownerless_identity", "high", "Zulu", "summary", []byte("[\"x\"]"), []byte("{\"a\":1}"), "fix", now.Add(time.Second))
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT f.scan_id, f.finding_id, f.type, f.severity, f.title, f.human_summary, f.path, f.evidence, COALESCE(f.remediation, ''), f.created_at
			 ...
			 WHERE f.scan_id = $1
			   AND f.finding_id = $2
			 LIMIT 1`)).
		WithArgs("scan-1", "finding-1").
		WillReturnRows(findingByScanRows)

	finding, err := store.GetFinding(defaultScopeContext(), " finding-1 ", " scan-1 ")
	if err != nil {
		t.Fatalf("get finding by scan: %v", err)
	}
	if finding.Title != "Zulu" {
		t.Fatalf("unexpected finding by scan: %+v", finding)
	}

	globalFindingRows := sqlmock.NewRows([]string{"scan_id", "finding_id", "type", "severity", "title", "human_summary", "path", "evidence", "remediation", "created_at"}).
		AddRow("scan-2", "finding-2", "ownerless_identity", "medium", "Bravo", "summary", []byte("[\"y\"]"), []byte("{\"b\":2}"), "fix", now.Add(2*time.Second))
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT f.scan_id, f.finding_id, f.type, f.severity, f.title, f.human_summary, f.path, f.evidence, COALESCE(f.remediation, ''), f.created_at
		 FROM findings f
		 ...
		 LIMIT 1`)).
		WithArgs("finding-2", "default", "default").
		WillReturnRows(globalFindingRows)

	finding, err = store.GetFinding(defaultScopeContext(), "finding-2", "")
	if err != nil {
		t.Fatalf("get finding without scan: %v", err)
	}
	if finding.ScanID != "scan-2" {
		t.Fatalf("unexpected finding without scan: %+v", finding)
	}
