...
func awsRuntimeEventFixtureRecords(accountID string, region string, fixtureState string, checkedAt time.Time) ([]AWSRuntimeEventRecord, []AWSRuntimeEventDiagnostic, []AWSRuntimeEventCoverageGap) {
	...
	records := []AWSRuntimeEventRecord{
		...
		awsRuntimeSignalFixtureRecord(accountID, region, "evt-access-analyzer-prod-runtime-reader", "access-analyzer", "access-analyzer.amazonaws.com", "Finding", "sts:AssumeRole", productionRunnerRole, role, "iam_role", "identrail-runtime-reader", "access-analyzer", base.Add(26*time.Minute), base.Add(34*time.Minute), "observed", 0.91, "account", fmt.Sprintf("arn:aws:access-analyzer:%s:%s:analyzer/identrail-fixture", region, accountID)),
	}
