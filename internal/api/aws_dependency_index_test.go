...
func TestGetAWSPlatformDependencyIndexBuildsCanonicalLedger(t *testing.T) {
	...
	if got, want := strings.Join(result.CompletedIssueRefs, ","), "#1473,#1474,#1475,#1476,#1477,#1478,#1479,#1480"; got != want {
		t.Fatalf("completed refs = %q, want %q", got, want)
	}
	wantReadyRefs := "#1481,#1482,#1483,#1484,#1485,#1486,#1487,#1488,#1489,#1490,#1491,#1492,#1493,#1494,#1495,#1496"
	...
	completedEKS := requireAWSPlatformDependencyIssue(t, result.Issues, "#1480")
	if completedEKS.ReadyForPR || completedEKS.DependencyStatus != awsPlatformIssueStateCompleted {
		t.Fatalf("expected #1480 to be completed after this PR, got %+v", completedEKS)
	}
	ready := requireAWSPlatformDependencyIssue(t, result.Issues, "#1481")
	if !ready.ReadyForPR || ready.DependencyStatus != awsPlatformIssueStateReady || len(ready.FailureReasons) != 0 {
		t.Fatalf("expected #1481 to be ready after #1480 completed, got %+v", ready)
	}
...
func TestRouterAWSPlatformDependencyIndex(t *testing.T) {
	...
	if !containsString(body.Index.CompletedIssueRefs, "#1480") || !containsString(body.Index.ReadyIssueRefs, "#1481") || body.Index.Status != awsPlatformDependencyStatusReady {
