...
func TestParseSQSSNSPolicyGrants(t *testing.T) {
	...
	if statementCount != 3 || len(grants) != 2 {
		t.Fatalf("expected 3 statements and 2 grants, got statements=%d grants=%+v", statementCount, grants)
	}
	if !grants[0].WildcardPrincipal || grants[0].Capabilities[0] != "publish" {
		t.Fatalf("expected public publish grant, got %+v", grants[0])
	}
	if !grants[1].HasCondition || !containsSQSSNSTestString(grants[1].Capabilities, "consume") {
		t.Fatalf("expected conditioned consume grant, got %+v", grants[1])
	}
}
