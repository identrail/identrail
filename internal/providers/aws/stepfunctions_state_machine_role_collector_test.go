...
func TestStepFunctionsDefinitionMetadataExtractsOnlySafeReferences(t *testing.T) {
	definition := `{"StartAt":"Charge","States":{"Charge":{"Type":"Task","Resource":"arn:aws:states:::lambda:invoke","Parameters":{"FunctionName":"arn:aws:lambda:us-east-1:123456789012:function:charge-card","Payload":{"card":"do-not-store"}},"Next":"Nested"},"Nested":{"Type":"Task","Resource":"arn:aws:states:::states:startExecution","Parameters":{"StateMachineArn":"arn:aws:states:us-east-1:123456789012:stateMachine:risk-check"},"End":true}}}`
	summary := stepFunctionsDefinitionMetadata(definition)
	if summary.Hash == "" {
		t.Fatal("expected hash")
	}
	if strings.Contains(strings.Join(summary.ResourceARNs, ","), "do-not-store") {
