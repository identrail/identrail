...
func TestRelationshipResolverDeterministicAcrossShuffledRawAssetInput(t *testing.T) {
	rawA := loadRawFixture(t, "k8s_service_account", "service_account_payments.json", "k8s:sa:apps:payments-api")
	rawB := loadRawFixture(t, "k8s_role", "cluster_role_cluster_admin.json", "k8s:role:cluster:cluster-admin")
	rawC := loadRawFixture(t, "k8s_role_binding", "role_binding_cluster_admin.json", "k8s:rb:cluster:payments-cluster-admin")
	rawD := loadRawFixture(t, "k8s_pod", "pod_payments.json", "k8s:pod:apps:payments-api-0")
