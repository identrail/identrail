...
func TestBuildScanServiceAWSSDKMode(t *testing.T) {
	...
	if composite, ok := scannerRunner.Collector.(*aws.AWSCompositeCollector); !ok {
		...
		assertAWSCompositeServiceNames(t, composite, []string{"iam", "ec2", "ecs", "lambda", "codebuild", "codepipeline", "eks"})
	}
	...
	if connectorComposite, ok := connectorAppScanner.Collector.(*aws.AWSCompositeCollector); !ok {
		...
		assertAWSCompositeServiceNames(t, connectorComposite, []string{"iam", "ec2", "ecs", "lambda", "codebuild", "codepipeline", "eks"})
	}
	if err := closeFn(); err != nil {
		t.Fatalf("close failed: %v", err)
	}
