...
Manual mode is intentionally limited. It lets a developer enter a tenant ID, workspace ID, and project ID directly, and it works only when `IDENTRAIL_AUTH_MANUAL_MODE=true`. Startup validation refuses to boot with manual mode enabled unless `IDENTRAIL_PUBLIC_BASE_URL` is a loopback origin (`http://localhost`, `http://127.0.0.1`, or `http://[::1]`), so it cannot be exposed accidentally outside local development; a deliberately non-production test deployment must opt in explicitly with `IDENTRAIL_AUTH_MANUAL_MODE_ALLOW_UNSAFE=true`. Startup also rejects this flag if WorkOS is configured, and emits a security warning whenever manual mode is active. The UI shows a "Dev Mode" banner whenever manual mode is active.
