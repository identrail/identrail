...
3. Each webhook event has a provider-issued ID. After signature validation (so a forged request cannot poison the ledger), the WorkOS handler atomically claims `(provider, event_id)` in the durable `webhook_events` table before applying any user-lifecycle side effect. The row carries a status so concurrent deliveries are handled correctly: the first delivery claims it (`processing`) and applies the side effects, then marks it `processed`. A duplicate that arrives **while the first is still in flight** is told to retry (HTTP 503 + `Retry-After`), never acknowledged — otherwise the provider could stop retrying before the effects are durably applied. Only a `processed` row turns a later duplicate, retry, or replay (within the signature tolerance window) into a no-op success. The check is durable across API restarts and shared by every API instance pointed at the same database. If a transient server-side failure prevents the side effect from being applied, the claim is rolled back so a provider retry can reprocess; deterministic failures (bad payload, identity conflict) are marked `processed` so identical retries stay no-ops. A `processing` row left behind by an instance that crashed mid-processing is reclaimable after a grace period so an event cannot be wedged forever. Each claim mints a random claim token; completion and rollback only act on the row if that token still matches, so a handler that overran the grace period and was superseded by a reclaiming retry cannot complete or erase the successor's in-flight claim. Completion and rollback both run on a context detached from the request so a client disconnect mid-processing cannot leave the row stuck. The idempotency record is written only after signature validation, and rows that predate this ledger are treated as already-processed. The ledger is bounded: rows older than a retention window that comfortably exceeds the provider's retry/replay window are opportunistically pruned on the claim path so the table cannot grow without limit.
