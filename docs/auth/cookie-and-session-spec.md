...
- **Idle timeout: 24 hours.** On every authenticated request, we set `idle_expires_at = now() + 24h`, capped by `absolute_expires_at`. If a request arrives after `idle_expires_at`, the session is rejected and the user is redirected to `/signin?return_to=...`.
